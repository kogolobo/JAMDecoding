...
class Constraint(ABC):
    ...
    def __init__(self, constraint_idx: int):
...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: List[int], constraint_idx: int):
        ...
        self.hashable_token_ids = tuple(token_ids)
    ...
    def copy(self, stateful=False):
        new_constraint = PhrasalConstraint(self.token_ids, self.constraint_idx)
...
class DisjunctiveConstraint(Constraint):
    ...
    def __init__(self, nested_token_ids: List[List[int]], constraint_idx: int):
        super(Constraint, self).__init__()
        ...
        self.hashable_token_ids = tuple(tuple(token_ids) for token_ids in nested_token_ids)
    ...
    def copy(self, stateful=False):
        new_constraint = DisjunctiveConstraint(self.token_ids, self.constraint_idx)
...
class ConstraintListState:
    ...
    def __init__(self, constraints: List[Constraint], ordered: bool):
    ...
    def get_complete_num(self):
        """
        Returns the number of completed constraints so far.
        """
        return len(self.complete_constraints)

    def get_complete_constraints(self):
        return tuple(constraint.hashable_token_ids for constraint in self.complete_constraints)

    def get_bank(self):
        # todo change this to consider type of satisfied the constraints?
        add = 0
    ...
    def copy(self, stateful=True):
        new_state = ConstraintListState(self.constraints, self.ordered)
        # we actually never though self.constraints objects throughout this process. So it's at initialization state.
    ...
    def is_ordered(self):
        current_constraint_order = self.complete_constraints + [self.inprogress_constraint] + self.pending_constraints
        current_constraint_order = [constraint.constraint_idx for constraint in current_constraint_order
                                    if constraint is not None]

        return sorted(current_constraint_order) == current_constraint_order
